...
import { getDocs, doc, onSnapshot, collection } from "firebase/firestore";
...
export const GamePage: React.FC<Props> = ({ colorScheme }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "games", gameId), (data) => {
      // getDocs(collection(db, `games/${gameId}/Comments`)).then((data) => {
    ...
    const unsub2=onSnapshot(collection(db, `games/${gameId}/Comments`),(data)=>{
      let dataArr:CommentType[]=[]
      data.docs.forEach((doc) => dataArr.push({ ...doc.data(), id: doc.id }))
      setComments([...dataArr])
    })
